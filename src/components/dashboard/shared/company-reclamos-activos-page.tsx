...
import { Eye, Search, X, Calendar, User, Download, Loader2 } from "lucide-react";
...
interface CompanyReclamosActivosPageProps {
  userRole: "owner" | "operador";
}

export function CompanyReclamosActivosPage({ userRole }: CompanyReclamosActivosPageProps) {
  ...
  const [downloadingType, setDownloadingType] = useState<'active' | 'all' | null>(null);
  ...
  const fetchReclamos = async () => {
    try {
      ...
      setFilteredReclamos(activeReclamos);
    } catch (error) {
  ...
  const handleDownloadReport = async (type: 'active' | 'all') => {
    try {
      ...
      if (!filename) {
        ...
        const tipoReporte = type === 'active' ? 'En curso' : 'Historico';
